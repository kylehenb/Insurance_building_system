...
import { usePlaybookState } from '@/lib/hooks/usePlaybookState'
...
// ── Pinned Playbook Cards ───────────────────────────────────────────────────
//
// Permanently pinned section at the top of the To Do list.
// Shows the current playbook step in a two-card layout with progress tracking.

function PinnedPlaybookCards({ jobId, tenantId, job }: { jobId: string; tenantId: string; job: JobDetails }) {
  const supabase = createBrowserClient(
    process.env.NEXT_PUBLIC_SUPABASE_URL!,
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!
  )
  ...
  useEffect(() => {
    async function load() {
      ...
      const approvedQuote = quotes.find(q => q.quote_type === 'inspection' && q.status === 'approved')
  ...
  function handleInlineAction(target: string) {
    // Handle inline actions (e.g., close job confirmation)
    if (target === 'close_job') {
      // TODO: Implement inline job close action
      console.log('Close job action triggered')
    }
  }

  function handleAction(action: { type: string; target?: string; label: string }) {
    if (action.type === 'navigate' && action.target) {
      handleNavigate(action.target)
    } else if (action.type === 'inline' && action.target) {
      handleInlineAction(action.target)
    }
  }
  ...
  return (
    ...
    >
      {/* Progress bar */}
      <div style={{ height: 4, background: '#e8ddd0' }}>
        <div
          style={{
            height: '100%',
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 11, color: '#9e998f' }}>
            {progress.complete}/{progress.total} steps complete
          </span>
        </div>
      </div>

      {/* Two-card layout */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5px', background: '#e4dfd8' }}>
        {/* Left card: Next step */}
        <div
          style={{
            background: '#fff',
        ...
        >
          ...
          <div style={{ fontSize: 13, color: '#7a7167', lineHeight: 1.5, marginBottom: 12 }}>
        ...
        >
          ...
          {currentStep.primaryAction && (
            <button
              onClick={() => handleAction(currentStep.primaryAction!)}
              style={{
                width: '100%',
                padding: '10px 16px',
                background: '#c9a96e',
          ...
          {currentStep.skipAllowed && (
            <div style={{ marginTop: 8 }}>
              {skipConfirm ? (
                <span style={{ fontSize: 11, color: '#9e998f' }}>
                  Skip &ldquo;{currentStep.label}&rdquo;?{' '}
                  <button
                    onClick={() => { setSkipConfirm(false); skipStep(currentStep.id) }}
...
function ActionCardsSection({ jobId, tenantId, job }: { jobId: string; tenantId: string; job: JobDetails }) {
  ...
  return (
    <>
      {/* Pinned Playbook Cards */}
      <PinnedPlaybookCards jobId={jobId} tenantId={tenantId} job={job} />
