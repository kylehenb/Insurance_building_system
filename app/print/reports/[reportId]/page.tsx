...
const ROOF_TEMPLATE = {
  ...
  roof_condition_fields: [
    ...
    { label: 'Gutter overflows:', field: 'gutter_overflows' },
        ...
        <div style={{ padding: '0 20px' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '16px', border: isLDR ? '1px solid #c8d8e0' : (isRoof ? '1px solid #e0dcd4' : '1px solid #e0dbd4'), borderRadius: '6px', overflow: 'hidden', fontSize: '11px' }}>
            <tbody>
              ...
              {isRoof && (
                <>
                  {dividerRow('Roof Report Details')}
                  <tr>
                    <td style={tdCell}>
                  ...
                  <tr>
                    <td style={tdCellLast}>
                      <div style={cellContentStyle}>
                        <span style={labelStyle}>Scope of Roof Report:</span>
              ...
              {/* Block 1 — Attendance (LDR only) */}
              {isLDR && (
                <>
                  {dividerRow('Attendance', getNextSectionNum())}
                  <tr>
                    <td style={tdCell}>
                      <div style={cellContentStyle}>
                        <span style={labelStyle}>Date attended:</span>
              ...
              {isRoof && (
                <>
                  {dividerRow('Roof Details')}
                  {(() => {
                    // Combine roof details and roof condition fields
                    const allFields = [...ROOF_TEMPLATE.roof_details_fields, ...ROOF_TEMPLATE.roof_condition_fields]
                    const rows: React.ReactNode[] = []
                    for (let i = 0; i < allFields.length; i += 3) {
                      const item1 = allFields[i]
                      const item2 = allFields[i + 1]
                      const item3 = allFields[i + 2]
                      const isLast = i + 3 >= allFields.length
              ...
              {/* Block 3 — Claim Damage Findings (Roof only) - REMOVED, moved to narrative sections */}

              {/* Block 4 — Maintenance (Roof only) - REMOVED, moved to narrative sections */}

              {/* Block 5 — Additional (Roof only) - REMOVED, moved to narrative sections */}
          ...
          {(() => {
            ...
            // Roof narrative sections - rendered with earthy deep red color scheme, white backgrounds with borders
            if (isRoof) {
              const roofSections: Array<{ key: string; title: string; renderBody: () => React.ReactNode; isHeader?: boolean }> = [
                {
                  key: 'claim_damage_findings_header',
                  title: 'Claim Damage Findings',
                  renderBody: () => null,
                  isHeader: true,
                },
                ...ROOF_TEMPLATE.damage_cause_fields.map(f => ({
                  key: f.field,
                  title: f.label.replace(':', ''),
                  renderBody: () => (
                    <div style={{ background: '#ffffff', border: '1px solid #e0dcd4', borderRadius: '5px', padding: '10px 12px', fontSize: '11.5px', color: '#1a1a1a', lineHeight: '1.65' }}>
                      {formatTextWithPreservedFormatting(tsf(report, f.field))}
                    </div>
                  ),
                })),
                ...ROOF_TEMPLATE.maintenance_fields.map(f => ({
                  key: f.field,
                  title: f.label.replace(':', ''),
                  renderBody: () => (
                    <div style={{ background: '#ffffff', border: '1px solid #e0dcd4', borderRadius: '5px', padding: '10px 12px', fontSize: '11.5px', color: '#1a1a1a', lineHeight: '1.65' }}>
                ...
                ...ROOF_TEMPLATE.additional_fields.map(f => ({
                  key: f.field,
                  title: f.label.replace(':', ''),
                  renderBody: () => (
                    <div style={{ background: '#ffffff', border: '1px solid #e0dcd4', borderRadius: '5px', padding: '10px 12px', fontSize: '11.5px', color: '#1a1a1a', lineHeight: '1.65' }}>
                ...
                if (section.isHeader) {
                  ...
                  nodes.push(
                    <div key={section.key} style={{ marginBottom: '16px' }}>
                      <div style={{ fontSize: '12px', letterSpacing: '1.3px', textTransform: 'uppercase', color: '#1a1a1a', fontWeight: '800', padding: '4px 10px', backgroundColor: '#f5f0e8', borderBottom: '1px solid #e0dcd4' }}>
                        {section.title}
                      </div>
                    </div>
                  )
                  ...
                  nodes.push(
                    <div key={section.key} style={{ marginBottom: '16px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '7px', marginBottom: '6px' }}>
                        <div style={{ width: '20px', height: '20px', border: '1.5px solid #1a1a1a', color: '#1a1a1a', fontSize: '9px', fontWeight: '700', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                          {num}
                        </div>
                        <div style={{ fontSize: '9px', letterSpacing: '1.3px', textTransform: 'uppercase', color: '#1a1a1a', fontWeight: '800' }}>
                          {section.title}
                        </div>
                      </div>
                      {section.renderBody()}
