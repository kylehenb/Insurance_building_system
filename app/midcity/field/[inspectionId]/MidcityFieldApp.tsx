...
export default function MidcityFieldApp({ initialData }: { initialData: InitialData }) {
  // API routes are shared with the IRC field app backend
  ...
  const runAILabels = async () => {
    ...
    try {
      const res = await fetch(`${base}/ai-label`, {
        ...
        body: JSON.stringify({ context: photoContext, photoCount: photos.length, jobContext: { lossType: initialData.lossType, insurer: 'Midcity', address: initialData.address } }),
      })
    ...
    <div className="fa-root">
      ...
      <div className="fa-header">
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <div className="fa-logo"><span>BC</span></div>
        ...
        <div className="fa-sc-body">
          ...
          <div className="fa-ai-dark" style={{ marginBottom: 0 }}>
            ...
            <div className="fa-ai-dark-body">
              <textarea
                className="fa-ai-dark-ta"
                placeholder="Describe your photos in order. e.g. 'First is the damaged ceiling in the living room, second shows the water stain on the wall…'"
                style={{ minHeight: 80 }}
                value={photoContext}
          ...
          <div className="fa-submitted-overlay">
            ...
            {submitPath === 'none' && (
              <div style={{ marginBottom: 14 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  <a
                    href="/midcity"
                    style={{ width: '100%', padding: '13px 16px', background: 'rgba(200,184,154,.15)', color: 'var(--beige)', border: '1px solid rgba(200,184,154,.3)', borderRadius: 6, fontFamily: 'var(--font-bebas-neue)', fontSize: 18, letterSpacing: 2, cursor: 'pointer', textAlign: 'center', display: 'block', textDecoration: 'none' }}
                  >
                    Back to Midcity Jobs
                  </a>
                </div>
              </div>
            )}
            ...
            {amendmentSent && (
              <div style={{ background: 'var(--green-light)', border: '1px solid #c5dfc9', borderRadius: 6, padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: 'var(--green)' }}>
                ...
                <div>
                  <div style={{ fontWeight: 500 }}>All done</div>
                  <div style={{ fontSize: 11, marginTop: 1 }}>Inspection submitted successfully</div>
                </div>
              </div>
            )}
          </div>
