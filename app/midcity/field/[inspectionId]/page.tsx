...
import MidcityFieldApp from './MidcityFieldApp'

interface Props {
  params: Promise<{ inspectionId: string }>
}

export default async function MidcityFieldAppPage({ params }: Props) {
  ...
  const { data: insp, error } = await service
    ...
    .select(`
      ...
      safety_confirmed_at, form_submitted_at, start_time,
      jobs!job_id (
      ...
      <div className="min-h-screen bg-[#f5f0e8] flex items-center justify-center p-4">
        <div className="bg-white rounded-lg border border-[#1a1a1a]/10 p-8 max-w-md">
          ...
          <p className="text-sm text-[#1a1a1a]/70 mb-6">
            This inspection could not be found or you don&apos;t have access to it.
          </p>
          <a
            href="/midcity"
  ...
  const initialData = {
    ...
    status: insp.status ?? 'appointment_confirmed',
    scheduledDate: insp.scheduled_date ?? null,
    scheduledTime: (insp as any).start_time ?? null,
    jobId: job?.id ?? null,
  ...
  return <MidcityFieldApp initialData={initialData} />
}
